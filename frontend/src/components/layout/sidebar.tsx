...
const navigation = [
  { label:"Overview", href:"/overview", icon:LayoutDashboard }, { label:"Calls", href:"/calls", icon:PhoneCall },
  { label:"Leads", href:"/leads", icon:UsersRound }, { label:"Appointments", href:"/appointments", icon:CalendarDays },
  { label:"Knowledge", href:"/knowledge", icon:BookOpenText }, { label:"Notifications", href:"/notifications", icon:MessageSquareText },
];
...
export function Sidebar({ open, close, session, newLeads }:{ open:boolean; close:()=>void; session:Session; newLeads:number }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <nav aria-label="Primary navigation"><p className="nav-label">Workspace</p>{navigation.map(item => <Link className={`nav-item ${pathname === item.href ? "active" : ""}`} href={item.href} onClick={close} key={item.href}><item.icon size={18} strokeWidth={1.8}/><span>{item.label}</span>{item.href === "/leads" && newLeads > 0 && <em>{newLeads}</em>}</Link>)}<p className="nav-label secondary">Manage</p><Link className={`nav-item ${pathname === "/settings" ? "active" : ""}`} href="/settings"><Settings size={18}/><span>Business settings</span></Link><span className="nav-item muted"><CircleHelp size={18}/><span>Help & support</span></span></nav>
      <Link className="sidebar-footer" href="/profile"><div className="avatar">{initials(name)}</div><div><strong>{name}</strong><span>{member.role}</span></div><ChevronDown size={15}/></Link>
    </aside>
  </>;
}
