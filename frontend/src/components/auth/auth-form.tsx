...
export function AuthForm({ mode }:{ mode:"login"|"signup" }) {
  ...
  async function submit(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiRequest(`/backend/api/v1/auth/${mode}/`, { method:"POST", body:JSON.stringify(mode === "signup" ? { name:form.get("name"), business_name:form.get("business_name"), email:form.get("email"), password, password_confirm:confirmation } : { email:form.get("email"), password, remember_me:form.get("remember_me") === "on" }) });
